...
class FocusArea(BaseModel):
    topic: str = Field(description="Project or experience area to focus on")
    rationale: str = Field(description="Why this area is worth probing")

class StrategistResponse(BaseModel):
    interview_strategy: str = Field(description="A concise strategy description")
    focus_areas: List[FocusArea] = Field(description="List of focus areas to explore")

STRATEGIST_PROMPT = ChatPromptTemplate.from_messages([
    ("system", """You are an expert Technical Interview Strategist.
...
{{
  "interview_strategy": "string - A concise strategy description",
  "focus_areas": [
    {{
      "topic": "string - Project or experience area to focus on",
      "rationale": "string - Why this area is worth probing"
    }}
  ]
}}"""),
    ("human", """INPUT DATA:
...
def question_strategist_node(state: AgentState) -> dict:
    ...
    chain = STRATEGIST_PROMPT | llm.with_structured_output(StrategistResponse, method="json_mode")
    ...
    try:
        ...
        response: StrategistResponse = chain.invoke(inputs)

        # COST TRACKING
        cost = 0.0
        try:
            from services.cost_tracker import CostTracker
            input_len = len(STRATEGIST_PROMPT.format(**inputs))
            output_len = len(str(response.model_dump()))
            cost = CostTracker.track_cost("Strategist", input_len//4, output_len//4)
        except:
            pass

        strategy_text = response.interview_strategy
        focus_areas_list = [
            f"{f.topic}: {f.rationale}"
            for f in response.focus_areas
        ]
